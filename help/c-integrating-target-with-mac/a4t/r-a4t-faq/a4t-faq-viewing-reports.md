...
For activities using [!DNL Analytics] as the reporting source (A4T), the goal metric will always use the "[!UICONTROL Increment Count & Keep User in Activity]" and "[!UICONTROL On Every Impression]" settings. This is *not* configurable. 
...
In case classification was done for that activity, and you still see an "Unspecified" row in the report, make sure the report isn't using a non-[!DNL Target] metric to display the data. Unless the report is using a [!DNL Target]-specific metric, that "Unspecified" row will contain events for calls that are not associated with [!DNL Target]. That row will not contain any [!DNL Target]-associated information (e.g. visitors/visits/impressions).
...
When a visitor qualifies for, views content, or converts in an A4T activity, [!DNL Target] sends event data to [!DNL Analytics], which allows [!DNL Analytics] to attribute conversion events and other clickstream events happening on the page to the relevant [!DNL Target] activities and experiences.
